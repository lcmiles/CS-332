...
./lcmiles_HW02 -e <linux command string>
OR
make run
make search s=<string pattern> d=<depth>
make verbose
make size limit L=<size limit>
make linux cmd e=<linux command string>
*/
...
//function prototypes
int traverseDirectory(char *base_path, const int root, const int verbose, const int file_size_limit, const char *linux_command);
int traverseDirectoryWithSearch(char *base_path, const int root, const int verbose, const int file_size_limit, const char *search_pattern, const int search_depth, const char *linux_command);
...
int main(int argc, char *argv[]) {
    ...
    char *path;
    char *ptr;
    char *linux_command = NULL;


    int opt;
    while ((opt = getopt(argc, argv, "vL:s:e:")) != -1) { //loop through command line options
        switch (opt)
        {
        case 'v': //verbose case
        ...
        case 's': //search case
            ...
            optind++;  
            break;
        case 'e': //run linux command case
            linux_command = optarg;
            break;
    ...
    if (search_pattern == NULL && search_depth == -1) { //if the search_pattern and search_depth variables are set to their default values, execute traverseDirectory()
        traverseDirectory(path, 0, verbose, file_size_limit,linux_command);
    }
    else traverseDirectoryWithSearch(path, 0, verbose, file_size_limit, search_pattern, search_depth,linux_command); //otherwise execute traverseDirectoryWithSearch()
        ...
        if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0) { //exclude the "." and ".." directories
            ...
            if (linux_command != NULL) {
                ...
                char command[10000];
                    ...
                    snprintf(command, sizeof(command), "%s %s", linux_command, path);
                    
                    system(command);
            }

            if (S_ISDIR(file_stat.st_mode)) { //if the file is a directory recursively call traverseDirectory()
                traverseDirectory(path, root + 1, verbose, file_size_limit, linux_command);
            }
        }
    }
    closedir(dir);
...
int traverseDirectoryWithSearch(char *base_path, const int root, const int verbose, const int file_size_limit, const char *search_pattern, const int search_depth, const char *linux_command) { //code is similar to traverseDirectory() except for file printing exclusion
    char path[1000];
    struct dirent *dp;
    DIR *dir = opendir(base_path);
    ...
    while ((dp = readdir(dir)) != NULL) {
        if (strcmp(dp->d_name, ".") != 0 && strcmp(dp->d_name, "..") != 0) {
            ...
            if (S_ISDIR(file_stat.st_mode)) {
                traverseDirectoryWithSearch(path, root + 1, verbose, file_size_limit, search_pattern, search_depth, linux_command);
            }
        }
    }
    closedir(dir);
